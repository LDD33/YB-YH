...
#pragma mark 跑马灯点击事件
    ...
    scl1.bounces = YES;
    scl1.showsHorizontalScrollIndicator = NO;
...
-(void)downData{
    //获取糖果
    [MyNetworkingManager DDPOSTResqust:@"/user/Assets/isreceiveSuger" withparameters:@{} withVC:self progress:^(NSProgress * _Nonnull progre) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary * dic = (NSDictionary *)responseObject;
        if ([dic[@"isreceive"] isEqualToString:@"false"]) {
            [CandyView showWithnum:[dic[@"suger"] intValue] delegate:self];
        }
...
//                if (lastV) {
//                    make.bottom.equalTo(lastV.mas_bottom).offset(200);
//                }
//                else
//                    make.bottom.equalTo(self.walletBtn.mas_bottom).offset(20);
//            }];
